...
from unitree_rl_lab.assets.robots.unitree import WHEEL_DOG_CFG as ROBOT_CFG
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=[".*_joint[1-3]"], scale=0.25, use_default_offset=True, clip={".*": (-100.0, 100.0)}
    )
    #joint_vel = mdp.JointVelocityActionCfg(
     #  asset_name="robot", joint_names=[], scale=10.0, use_default_offset=True, clip={".*": (-100.0, 100.0)}
      # asset_name="robot", joint_names=[".*_joint4"], scale=10.0, use_default_offset=True, clip={".*": (-100.0, 100.0)}
   # )
...
class RewardsCfg:
    ...
    track_lin_vel_xy = RewTerm(
        func=mdp.track_lin_vel_xy_exp, weight=0.1, params={"command_name": "base_velocity", "std": math.sqrt(0.25)}
    )
    ...
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-5.0e-6 )
    ...
    feet_air_time = RewTerm(
        ...
        weight=0.1,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_link4"),
    ...
    air_time_variance = RewTerm(
        ...
        weight=-1.0,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_link4")},
    )
    feet_slide = RewTerm(
        func=mdp.feet_slide,
        weight=-0.1,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=".*_link4"),
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=".*_link4"),
        },
    )
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "threshold": 1,
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=["body_link", ".*_link1", ".*_link2", ".*_link3"]),
        },
    )
